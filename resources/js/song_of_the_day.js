...
let attempts = 0; // shakes counter
const maxAttempts = 3; // shakes allowed
...
window.revealSong = function () {
  ...
  setTimeout(() => {
    ...
    // reset rating and load previous
    resetRating();
...
window.setupPlayerControls = function () {
  ...
  audio.addEventListener("timeupdate", () => {
    ...
    progressBar.max = audio.duration; // correct max value checkup
    currentTimeLabel.textContent = formatTime(audio.currentTime);
    durationLabel.textContent = formatTime(audio.duration);
  });

  progressBar.addEventListener("input", () => {
    audio.currentTime = progressBar.value; //  seek
  });
